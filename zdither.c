...
#if defined(TGL_FEATURE_1_BIT)

#if TGL_FEATURE_RENDER_BITS != 16
#error TGL_FEATURE_1_BIT is not supported for this value of TGL_FEATURE_RENDER_BITS
#endif

#define DITHER_THRESHOLD 128  // Threshold for dithering

void ZB_ditherFrameBuffer_1bit(ZBuffer *zb, unsigned char *buf, int linesize) {
    unsigned short *pp;
    unsigned char *dest;
    int x, y;
    ...
    for (y = 0; y < zb->ysize; y++) {
        dest = buf + y * linesize;
        pp = zb->pbuf + y * zb->xsize;

        for (x = 0; x < zb->xsize; x++) {
            unsigned short pixel = pp[x];
            ...
            int grayscale = (r << 3) * 0.299 + (g << 2) * 0.587 + (b << 3) * 0.114;

            // Thresholding to 1-bit
            if (grayscale > DITHER_THRESHOLD) {
                dest[x >> 3] |= (0x80 >> (x & 7));  // Set bit for white
            } else {
                dest[x >> 3] &= ~(0x80 >> (x & 7)); // Clear bit for black
            }
        }
    }
}

#endif /* TGL_FEATURE_1_BIT */
